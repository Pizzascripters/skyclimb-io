...
module.exports = function(world, map, players){
  ...
  for(var i in players){
    ...
    p.isGrounded = true;
    for(var i in map) p.isGrounded |= Matter.SAT.collides(body, map[i]).collided;
    p.isGrounded &= body.velocity.y < 0.1;
    p.isGrounded &= body.velocity.y > -0.1;
    ...
    if(p.keyboard.jump && p.isGrounded) Matter.Body.setVelocity(body, {x: body.velocity.x, y: -JUMP_VELOCITY});
